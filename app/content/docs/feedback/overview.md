...
Every session flows through an automated pipeline: it is captured from a source, optionally handled by the AI support agent, classified with tags, reviewed by the PM agent, and potentially converted into an engineering issue.

## What is a Feedback Session?

A feedback session contains:

- **Messages**: The full conversation history between the customer and the support agent (or human agent)
...
A new session starts in the **active** status when the first message is received. During this phase, the AI support agent responds to customer messages in real time using the project's knowledge packages.
