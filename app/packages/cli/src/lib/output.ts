...
export function formatResourceTypes(): string {
  return [
    ...
    `  ${label('Add:')} name, type (required), description, goals, parent_id`,
    `  ${label('Update:')} name, type, description, goals, parent_id`,
    `  ${label('Content:')} a scope's content is the knowledge attached to it (see knowledge type)`,
    '',
    `${BOLD}knowledge${RESET}`,
    '  Reference docs (websites, docs portals, Notion pages, uploaded files, raw text) attached to a product scope.',
    `  ${label('Defaults to the project root scope when --scope is omitted.')}`,
    `  ${label('List:')} hissuno list knowledge [--scope <id>]`,
    `  ${label('Add:')} hissuno add knowledge --type <type> [--scope <id>] [--url|--content ...]`,
    '',
    `${BOLD}codebase${RESET}`,
    '  Project codebases (GitHub repositories). First-class entity.',
    `  ${label('Add:')} repo (required), branch, name, description, analysis_scope, scope`,
    `  ${label('List:')} hissuno list codebases`,
    `  ${label('Sub-commands:')} hissuno sync codebase <id>, hissuno remove codebase <id>`,
  ].join('\n')
}
