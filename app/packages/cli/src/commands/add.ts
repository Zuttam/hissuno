...
async function addKnowledge(opts: AddOpts): Promise<{ data: Record<string, unknown>; scopeId: string }> {
  const scopeId =
    (opts.scope as string) ||
    (await input({ message: 'Scope ID or slug:', validate: (v) => v.length > 0 || 'Required' }))
  if (!scopeId) {
    error('--scope is required for knowledge.')
    process.exit(1)
  }
...
export const addCommand = new Command('add')
  ...
  .option('--scope <id>', 'Scope ID or slug to attach the resource to (codebase, knowledge)')
  ...
  .action(async (type, opts, cmd) => {
    ...
    switch (type) {
      ...
      case 'knowledge': {
        ...
        data = result.data
        apiEndpoint = `/api/product-scopes/${result.scopeId}/knowledge`
        break
      }
      default:
