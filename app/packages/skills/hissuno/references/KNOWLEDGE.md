...
Reference documents (websites, docs portals, Notion pages, uploaded files, raw text) attached to a product scope. Knowledge is **not a top-level resource** - every entry is namespaced by its owning scope. The CLI surface always requires `--scope`.
...
```bash
hissuno list knowledge --scope <scope-slug-or-id>
```

`--scope` is required. Returns all knowledge entries linked to the scope via `entity_relationships`.
...
```bash
hissuno add knowledge --scope <slug> --type <type> [...flags]
```

Required flags: `--scope`, `--type`. Knowledge types: `website`, `docs_portal`, `uploaded_doc`, `raw_text`, `notion`.
...
hissuno add knowledge --scope auth --type website --url https://...       # Add a website
