...
/**
 * Keyword search across project codebases. Matches on name, repository_url,
 * and description. Returns the limit most-recent matches.
 */
export async function searchCodebases(
  projectId: string,
  query: string,
  limit: number = 10,
  ...
  const rows = await db
    .select({
      ...
      description: codebases.description,
    })
    .from(codebases)
    .where(
      and(
        ...
        or(
          ilike(codebases.name, s),
          ilike(codebases.repository_url, s),
          ilike(codebases.description, s),
        ),
      ),
    )
  ...
  return rows.map((r) => ({
    id: r.id,
    name: r.name || r.repository_url || 'Untitled codebase',
    snippet: [r.repository_url, r.repository_branch].filter(Boolean).join(' · '),
  }))
}
