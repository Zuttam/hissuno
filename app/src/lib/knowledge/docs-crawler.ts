...
import { URL } from 'url'
import { assertSafeUrl } from '@/lib/security/url-validation'
...
export async function crawlDocsPortal(entryUrl: string, options: CrawlOptions = {}): Promise<CrawlResult[]> {
  ...
  // Validate entry URL against SSRF
  try {
    await assertSafeUrl(normalizedEntry)
  } catch (err) {
    const message = err instanceof Error ? err.message : 'URL validation failed'
    return [{ url: entryUrl, title: '', content: '', error: message }]
  }
  ...
  while (queue.length > 0 && results.length < opts.maxPages) {
    ...
    // Validate each URL against SSRF before fetching
    try {
      await assertSafeUrl(url)
    } catch {
      console.log(`[docs-crawler] Blocked unsafe URL: ${url}`)
      continue
    }
