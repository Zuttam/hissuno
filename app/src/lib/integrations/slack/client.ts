...
export class SlackClient {
  ...
  async removeReaction(params: {
    ...
    try {
      const result = await this.request<Record<string, never>>('reactions.remove', {
        channel: params.channel,
        timestamp: params.timestamp,
        name: params.name,
      })
      ...
      if (!result.ok) {
        ...
        console.warn('[SlackClient.removeReaction] Error:', result.error)
        return { ok: false, error: result.error }
      }

      return { ok: true }
    } catch (error) {
      console.warn('[SlackClient.removeReaction] Exception:', error)
      return { ok: false, error: error instanceof Error ? error.message : 'Unknown error' }
    }
  }
