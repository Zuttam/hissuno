...
import { setSessionHumanTakeover } from '@/lib/db/queries/sessions'
...
export async function handleSlackEvent(payload: SlackEventPayload): Promise<void> {
  ...
  const tokenInfo = await getSlackBotToken(teamId)
...
async function handleAppMention(params: {
  ...
  const { event, projectId, botUserId, slackClient, teamId } = params
  ...
  let channel = await getSlackChannelWithMode(teamId, event.channel)
  ...
  if (!channel) {
    ...
    const workspaceTokenRows = await db
      .select({ id: slackWorkspaceTokens.id, workspace_domain: slackWorkspaceTokens.workspace_domain })
      .from(slackWorkspaceTokens)
      .where(eq(slackWorkspaceTokens.workspace_id, teamId))
    ...
    const newChannel = await getOrCreateSlackChannel({
    ...
    channel = await getSlackChannelWithMode(teamId, event.channel)
    if (!channel) {
...
async function handleMessage(params: {
  ...
  const threadSession = await getThreadSession(channel.id, event.thread_ts)
  ...
  if (threadSession) {
    ...
    if (decision.shouldRespond) {
      ...
      await updateThreadSessionResponder(threadSession.id, 'user')
...
async function handlePassiveModeMessage(params: {
  ...
  const { event, channel, projectId, slackClient, teamId } = params
...
async function handleBotJoinedChannel(params: {
  ...
  const { event, slackClient, teamId } = params
  ...
  const result = await getOrCreateSlackChannel({
    workspaceTokenId: workspaceToken.id,
    channelId,
    channelName: channelInfo?.name || null,
    channelType: channelInfo?.is_private ? 'private_channel' : 'channel',
