...
export async function sendHumanNeededNotification(
  ...
  try {
    ...
    const [project] = await db
      .select({ user_id: projects.user_id, name: projects.name })
      .from(projects)
    ...
    if (shouldSlack) {
      try {
        ...
        const slackText = [
          ...
          ``,
          `The Hissuno Agent has flagged this session for human takeover.`,
          ``,
        ...
        if (slackResult.ok && slackResult.channelId) {
          ...
          await setSessionHumanTakeoverNotification({
            sessionId,
