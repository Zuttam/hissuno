...
import { userProfiles, projects, slackWorkspaceTokens } from '@/lib/db/schema/app'
  ...
  try {
    ...
    const [profile] = await db
      .select({
        ...
        slack_notification_channel: userProfiles.slack_notification_channel,
      })
      .from(userProfiles)
    ...
    if (userProjects.length > 0) {
      ...
      slackAvailable = tokens.length > 0
