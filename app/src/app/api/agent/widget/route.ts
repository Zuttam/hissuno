...
/**
 * GET /api/agent/widget?projectId=xxx
 *
...
export async function GET(request: NextRequest) {
  if (!isSupabaseConfigured()) {
    console.error('[agent/widget] Supabase must be configured')
  ...
  try {
    ...
    if (!isOriginAllowed(origin, project.allowed_origins)) {
      console.warn(`[agent/widget] Origin not allowed: ${origin} for project ${projectId}`)
    ...
    const response = NextResponse.json({
      ...
      tokenRequired: project.widget_token_required,
    })
    ...
    console.error('[agent/widget] unexpected error', error)
