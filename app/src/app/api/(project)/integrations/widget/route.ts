...
/**
 * GET /api/integrations/widget?projectId=...
 *
 * Get widget settings for a project (authenticated, dashboard use).
 * Returns default values if no settings exist.
 */
export async function GET(request: NextRequest) {
  if (!isDatabaseConfigured()) {
    console.error('[integrations.widget.get] Database must be configured')
  ...
  try {
    ...
    console.error('[integrations.widget.get] unexpected error', error)
    return NextResponse.json({ error: 'Failed to load widget settings.' }, { status: 500 })
  }
}

/**
 * PATCH /api/integrations/widget?projectId=...
 *
 * Update widget settings for a project (authenticated, dashboard use).
 * Accepts both prefixed (widget_*) and unprefixed field names for backwards compatibility.
 */
export async function PATCH(request: NextRequest) {
  if (!isDatabaseConfigured()) {
    console.error('[integrations.widget.patch] Database must be configured')
    return NextResponse.json({ error: 'Database must be configured.' }, { status: 500 })
  }
  ...
  try {
    ...
    console.error('[integrations.widget.patch] unexpected error', error)
