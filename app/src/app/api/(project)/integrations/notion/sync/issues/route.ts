/**
 * Notion issue sync API route.
 * GET - Trigger manual issue sync with SSE progress streaming
 */
...
/**
 * GET /api/integrations/notion/sync/issues?projectId=xxx
 * Trigger manual issue sync with SSE progress streaming
 */
export async function GET(request: NextRequest) {
  ...
  return createSSEStreamWithExecutor<BaseSSEEvent>({
    logPrefix: '[notion-sync-issues.stream]',
    executor: async ({ emit, close, isClosed }) => {
      emit(createSSEEvent('connected', { message: 'Starting issue sync...' }))

      try {
        await syncNotionIssues(projectId, (event) => {
          if (isClosed()) return
        ...
        if (!isClosed()) {
          emit(createSSEEvent('complete', { message: 'Issue sync complete.' }))
        }
      } catch (error) {
        console.error('[notion-sync-issues.stream] Error:', error)
        if (!isClosed()) {
