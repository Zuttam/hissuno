...
import { listCustomFieldDefinitions, createCustomFieldDefinition } from '@/lib/db/queries/custom-field-definitions'
import { isDatabaseConfigured } from '@/lib/db/config'
import { CUSTOM_FIELD_TYPES } from '@/types/ontology'
import type { EntityType } from '@/lib/db/queries/types'

const VALID_ENTITY_TYPES = ['company', 'contact', 'issue', 'session', 'knowledge_source', 'product_scope'] as const

export const runtime = 'nodejs'

/**
 * GET /api/settings/ontology/fields?projectId=...
 */
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const entityType = (searchParams.get('entity_type') as EntityType) ?? undefined
    ...
    console.error('[ontology-fields.list] unexpected error', error)
    return NextResponse.json({ error: 'Unable to load custom fields.' }, { status: 500 })
  }
}

/**
 * POST /api/settings/ontology/fields?projectId=...
 */
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.error('[ontology-fields.post] unexpected error', error)
