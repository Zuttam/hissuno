...
import { triggerSessionReview, sessionReviewStreamUrl } from '@/lib/api/sessions'
...
export default function ProjectSessionsPage() {
  ...
  const handleBatchAnalyze = useCallback(async () => {
    ...
    await batchAsync.executeBatch(items, async (session, signal) => {
      // Trigger the review
      const res = await triggerSessionReview(projectId!, session.id, signal)
      if (!res.ok) {
      ...
      await new Promise<void>((resolve, reject) => {
        const es = new EventSource(sessionReviewStreamUrl(projectId!, session.id))
