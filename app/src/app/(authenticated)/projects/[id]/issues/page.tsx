...
export default function ProjectIssuesPage() {
  ...
  const handleBatchAnalyze = useCallback(async () => {
    ...
    await batchAsync.executeBatch(items, async (issue, signal) => {
      const res = await fetch(`/api/projects/${projectId}/issues/${issue.id}/analyze`, {
        method: 'POST',
        signal,
      })
      if (!res.ok) {
        const body = await res.json().catch(() => null)
        throw new Error(body?.error ?? 'Failed to trigger analysis')
      }

      const { runId } = await res.json()

      await new Promise<void>((resolve, reject) => {
        const es = new EventSource(`/api/projects/${projectId}/issues/${issue.id}/analyze/stream?runId=${runId}`)
  ...
  const handleBatchGenerateSpec = useCallback(async () => {
    const items = selection.selectedItems
    if (!projectId) return

    await batchAsync.executeBatch(items, async (issue, signal) => {
      const res = await fetch(`/api/projects/${projectId}/issues/${issue.id}/generate-spec`, {
        method: 'POST',
        signal,
      })
      if (!res.ok) {
        const body = await res.json().catch(() => null)
        throw new Error(body?.error ?? 'Failed to trigger spec generation')
      }

      const { runId } = await res.json()

      await new Promise<void>((resolve, reject) => {
        const es = new EventSource(`/api/projects/${projectId}/issues/${issue.id}/generate-spec/stream?runId=${runId}`)
        ...
        es.addEventListener('error', (event) => {
          ...
          try {
            const data = JSON.parse((event as MessageEvent).data)
            reject(new Error(data.error ?? 'Spec generation failed'))
          } catch {
            reject(new Error('Spec generation failed'))
          }
        })
        ...
        <BatchActionBar
          ...
          actions={[
            ...
            },
            {
              ...
              onClick: () => void handleBatchGenerateSpec(),
              disabled: batchAsync.isRunning || selection.selectedCount > 20,
            },
            {
              label: 'Export',
