...
export function GongConfigDialog({
  ...
  const fetchStatus = useCallback(async () => {
    ...
    setError(null)
    try {
      const response = await fetch(`/api/integrations/gong?projectId=${projectId}`)
      if (!response.ok) {
  ...
  const handleTestConnection = async () => {
    ...
    try {
      const response = await fetch('/api/integrations/gong/test', {
        method: 'POST',
  ...
  return (
    <Dialog open={open} onClose={onClose} title="Gong Integration" size="2xl">
