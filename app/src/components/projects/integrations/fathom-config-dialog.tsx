...
export function FathomConfigDialog({
  ...
  const [status, setStatus] = useState<FathomStatus>({ connected: false })
  ...
  const fetchStatus = useCallback(async () => {
    ...
    setError(null)
    try {
      const response = await fetchFathomStatus(projectId)
      if (!response.ok) {
        throw new Error('Failed to load Fathom status')
      }
      ...
      setError(err instanceof Error ? err.message : 'Failed to load Fathom status')
    } finally {
      setIsLoading(false)
    }
  }, [projectId])
  ...
  const handleConnect = async () => {
    ...
    try {
      ...
      const response = await connectFathom({
        projectId,
        apiKey: apiKey.trim(),
        syncFrequency,
      ...
      setApiKey('')
      setSuccessMessage('Connected to Fathom successfully!')
      onStatusChanged?.()
      await fetchStatus()
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Failed to connect Fathom')
    } finally {
  ...
  const handleDisconnect = async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Failed to disconnect Fathom')
    } finally {
  ...
  const saveSettings = async (): Promise<{ success: boolean }> => {
    ...
    const response = await updateFathomSettings(projectId, {
      syncFrequency,
  ...
  const handleStopSync = () => {
    ...
    setIsSyncing(false)
    setSuccessMessage(`Sync stopped. ${synced} meeting${synced === 1 ? '' : 's'} synced.`)
  ...
  const handleSync = async () => {
    ...
    const eventSource = new EventSource(fathomSyncUrl(projectId, mode))
  ...
  return (
    <Dialog open={open} onClose={onClose} title="Fathom Integration" size="xxl">
        ...
        {isLoading ? (
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-3 border-t border-[color:var(--border-subtle)] pt-4">
                  ...
                  <label className="flex items-start gap-2 cursor-pointer">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-[color:var(--text-tertiary)]">
                        Only import meetings since {formatDate(status.lastSyncAt)}
                      </p>
                    </div>
                  </label>
                  <label className="flex items-start gap-2 cursor-pointer">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-[color:var(--text-tertiary)]">
                        Re-scan all meetings from your configured date range. Already imported sessions will be skipped.
                      </p>
                    </div>
                  </label>
                </div>
              ...
              <p className="text-xs text-[color:var(--text-tertiary)]">
                Manually trigger a sync to import meeting transcripts from Fathom.
                {status.nextSyncAt && (
                  <>
                    {' '}
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-1">
                ...
                <p className="text-xs text-[color:var(--text-tertiary)]">
                  Find this in your Fathom account under Settings &gt; API.
                </p>
              </div>
              ...
              <div className="space-y-2">
                ...
                <p className="text-xs text-[color:var(--text-tertiary)]">
                  Only sync meetings within this date range. Leave empty to sync all.
                </p>
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1">
