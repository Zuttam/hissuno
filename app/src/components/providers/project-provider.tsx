...
import { listProjects as fetchProjectsApi, fetchProject as fetchProjectApi } from '@/lib/api/projects'
...
export function ProjectProvider({ children }: ProjectProviderProps) {
  ...
  const fetchProject = useCallback(async (id: string) => {
    try {
      const payload = await fetchProjectApi<{ project: ProjectRow }>(id)
      setProject(payload.project ?? null)
    } catch (error) {
      console.error('[ProjectProvider] failed to fetch project', id, error)
      setProject(null)
      storeProjectId(null)
  ...
  const fetchProjects = useCallback(async () => {
    ...
    try {
      const payload = await fetchProjectsApi<{ projects: ProjectRow[] }>()
