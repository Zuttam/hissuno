...
function useCopyToClipboard(text: string) {
  ...
  const handleCopy = useCallback(async () => {
    try {
      ...
      console.error('[brief-view] failed to copy:', err)
    }
  }, [text])
...
function downloadAsMarkdown(content: string, title?: string) {
  ...
  a.href = url
  a.download = `brief-${title?.toLowerCase().replace(/\s+/g, '-') || 'product'}.md`
  a.click()
...
export function BriefView({ brief, issueTitle }: BriefViewProps) {
  const [isExpanded, setIsExpanded] = useState(false)
  const [isDialogOpen, setIsDialogOpen] = useState(false)
  const { copied, handleCopy } = useCopyToClipboard(brief)
    ...
    <div className="flex flex-col gap-2 py-2">
      ...
      >
        ...
        <div className="absolute right-2 top-2 z-10 flex gap-1">
          ...
          <button
            type="button"
            onClick={() => downloadAsMarkdown(brief, issueTitle)}
            className="flex h-7 w-7 items-center justify-center rounded-[4px] border border-[color:var(--border-subtle)] bg-[color:var(--surface)] text-[color:var(--text-secondary)] transition hover:bg-[color:var(--surface-hover)]"
        ...
        <div className="p-4 pr-32">
          <MarkdownContent content={brief} className="text-sm" />
        </div>
...
function BriefDialog({ open, onClose, brief, issueTitle }: BriefDialogProps) {
  const { copied, handleCopy } = useCopyToClipboard(brief)

  return (
    <Dialog open={open} onClose={onClose} title="Brief" size="xxl">
