...
// Use fake timers to make Date.now() and new Date() deterministic across all tests
...
describe('PM Review Trigger (Decoupled)', () => {
  describe('Review Triggered for Closed Sessions', () => {
    ...
    it('should not re-trigger review for session with skipped review', () => {
      // Skipped reviews (workflow not configured) are terminal — excluded from cron
      const closedSessions = [
        createMockSession({ id: 'session-1', status: 'closed' }),
      ]
