...
// parseSessionTaggingResponse, parsePMReviewResponse, parsePMAgentResponse
...
describe('parsePMAgentResponse (legacy)', () => {
  it('maps created action correctly', () => {
    const response = 'Created issue for bug in checkout'
    const result = parsePMAgentResponse(response)
    expect(result.action).toBe('created')
    expect(result.classification).toBe('bug')
  })

  it('maps skipped to skip classification', () => {
    const response = 'This is a resolved Q&A session.'
    const result = parsePMAgentResponse(response)
    expect(result.action).toBe('skipped')
    expect(result.classification).toBe('skip')
  })

  it('maps upvoted action correctly', () => {
    const response = 'Upvoted existing issue about checkout failure.'
    const result = parsePMAgentResponse(response)
    expect(result.action).toBe('upvoted')
  })
})
