...
function setupSupabaseMock(sessionData: { is_human_takeover: boolean } | null) {
  mockSingle.mockResolvedValue({ data: sessionData, error: null })
...
describe('decideIfShouldRespond', () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks()
    setupSupabaseMock({ is_human_takeover: false })
  })
  ...
  describe('human takeover mode (priority 0)', () => {
    it('should SKIP when session is in human takeover mode', async () => {
      setupSupabaseMock({ is_human_takeover: true })
