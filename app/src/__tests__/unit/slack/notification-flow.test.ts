...
function setupProjectLookup(userId: string, projectName: string) {
  // Drizzle chain: db.select({...}).from(projects).where(eq(...)).limit(1)
  mockDbLimit.mockResolvedValue([{ user_id: userId, name: projectName }])
...
describe('sendHumanNeededNotification', () => {
  ...
  describe('error handling', () => {
    it('should not throw when project lookup fails', async () => {
      // Drizzle returns empty array when not found
