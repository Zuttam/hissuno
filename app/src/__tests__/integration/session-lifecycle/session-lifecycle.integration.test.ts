 ...
 *
 * To run these tests:
 * - Ensure .env.local is configured with valid Supabase credentials
 ...
 * 2. A running Supabase instance (local or cloud)
 ...
 *
 * If Supabase is not configured, these tests will be skipped automatically.
 */
...
let createAdminClientFn: (() => ReturnType<typeof import('@/lib/supabase/server').createAdminClient>) | null = null

// Lazy load Supabase client only when tests actually run
async function getAdminClient() {
  if (!createAdminClientFn) {
    const mod = await import('@/lib/supabase/server')
    createAdminClientFn = mod.createAdminClient
  }
...
async function createTestSession(overrides: {
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      idle_prompt_sent_at: overrides.idle_prompt_sent_at ?? null,
      scheduled_close_at: overrides.scheduled_close_at ?? null,
      goodbye_detected_at: overrides.goodbye_detected_at ?? null,
      is_archived: false,
    })
...
async function simulateIdlePromptLogic(sessionId: string, idleTimeoutMinutes = 5, responseTimeoutSeconds = 60) {
  ...
  return { sent: true, messageId, scheduledCloseAt }
}
