 ...
 * - Generates product briefs when threshold is met
 *
...
import { projects, projectSettings, sessions, issues, entityRelationships } from '@/lib/db/schema/app'
...
async function setupTestProject(): Promise<string> {
  // Get the current user ID from an existing project
...
async function createSeedIssue(
  ...
  if (!issue) {
    throw new Error('Failed to create seed issue')
  }

  testIssueIds.push(issue.id)
  return issue as unknown as IssueRecord
}
...
async function cleanupTestData() {
  // Delete entity_relationships links first (foreign key constraint)
  if (testIssueIds.length > 0) {
    await db.delete(entityRelationships).where(inArray(entityRelationships.issue_id, testIssueIds))
  }
  if (testSessionIds.length > 0) {
    await db.delete(entityRelationships).where(inArray(entityRelationships.session_id, testSessionIds))
  }

  // Delete test issues
  if (testIssueIds.length > 0) {
    await db.delete(issues).where(inArray(issues.id, testIssueIds))
  }

  // Delete test sessions
  if (testSessionIds.length > 0) {
    await db.delete(sessions).where(inArray(sessions.id, testSessionIds))
  }

  // Delete test project settings and project
  if (testProjectId) {
    await db.delete(projectSettings).where(eq(projectSettings.project_id, testProjectId))
