...
export const pmDecision = createStep({
  ...
  execute: async ({ inputData, mastra, writer }) => {
    ...
    const { sessionId, projectId, tags, messages, settings, similarIssues, productScopeContext } = inputData
...
${productScopeContext.description ? `Description: ${productScopeContext.description}` : ''}
${productScopeContext.matchedGoalText ? `Matched goal: ${productScopeContext.matchedGoalText}` : 'No specific goal matched'}
...
{"action": "create", "newIssue": {"type": "bug|feature_request|change_request", "title": "Issue title", "description": "Detailed description with quotes", "priority": "low|medium|high"}}`
      ...
      // Create runtime context so PM agent can use knowledge tools
