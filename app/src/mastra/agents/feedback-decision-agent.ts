 ...
 */
export const feedbackDecisionAgent = new Agent({
  name: 'Feedback Decision Agent',
  instructions: `
...
You have access to the project's knowledge base through knowledge tools. Before making a decision, use them to understand product context:
...
- Check if a requested feature already exists (user may not know about it)
- Understand product context to write better issue descriptions
- Assess impact more accurately by understanding affected product scopes
...
`,
  ...
  // Issue tools for standalone use + analysis knowledge tools for product context queries
  tools: Object.fromEntries([...issueTools, ...analysisKnowledgeTools].map((tool) => [tool.id, tool])),
})
